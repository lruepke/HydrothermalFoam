...
# read mesh data
x,y,z, edges=foamToVTK.readPatchMesh(caseDir,patchname,time,coord2km=False,depthPositive=True,eval_patchIndex='((z==np.max(z)) & ((x<=300) | (x>3000)) & (y>=-3925))')

# figure
fig=plt.figure(figsize=scifig.figsize_cm(24, 9))
ax=plt.gca()
plt.axis('scaled')
# plot mesh
scifig.polyplot(ax,x,y,edges,linewidth=0.1, color='c')
x2,y2,z2, edges2=foamToVTK.readPatchMesh(caseDir,patchname,time,coord2km=False,depthPositive=True,eval_patchIndex='((z==np.max(z)) & (((x>1000) & (x<3000)) | (y<=-3925)))')
...
ax.legend(ncol=3,labelspacing=1,loc='upper center', bbox_to_anchor=(0,0.35,1,1))
# read bottom T
triangles,T=foamToVTK.readPatch(caseDir,'bottom','10','T',coord2km=False,depthPositive=True)
triangles.y=-triangles.y*10+4000
CS=ax.tripcolor(triangles,T,cmap='rainbow')
# plot colorful legend
ax_cs_legend=ax.inset_axes([0.61,1.15,0.05,0.02], transform=ax.transAxes,zorder=1)
ax_cs_legend.tripcolor(triangles,T,cmap='rainbow')
...
# color bar
ax_cb=ax.inset_axes([0.55,0.2,0.35,0.02], transform=ax.transAxes,zorder=1)
...
ax.annotate(txt, xy=(100,3400), xytext=(400,3400),color='c',ha='left',va='center', arrowprops=dict(facecolor='black',arrowstyle='->'))
ax.annotate(txt, xy=(3900,3400), xytext=(3600,3400),color='c',ha='right',va='center', arrowprops=dict(facecolor='black',arrowstyle='->'))
...
ax.xaxis.set_minor_locator(MultipleLocator(80))
