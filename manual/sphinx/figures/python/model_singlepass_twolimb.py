...
# read mesh data
x,y,z, edges=foamToVTK.readPatchMesh(caseDir,patchname,time,coord2km=False,depthPositive=True,eval_patchIndex='( (z==np.max(z)) & ( ((x<=300) & (y>=-3925)) | ((x>600) & (x<=(2000-75)))))') 
...
x2,y2,z2, edges2=foamToVTK.readPatchMesh(caseDir,patchname,time,coord2km=False,depthPositive=True,eval_patchIndex='((z==np.max(z)) & ((x>=(2000-75)) | (y<=-3925)))')
...
# read bottom T
triangles,T=foamToVTK.readPatch(caseDir,'bottom','129','T',coord2km=False,depthPositive=True)
...
ax.annotate(txt, xy=(1250,3050), xytext=(1000,3400),color='c',ha='left',va='center', arrowprops=dict(facecolor='black',arrowstyle='->'))
