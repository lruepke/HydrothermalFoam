...
// right side
Point(20) = {xmax+(xmax-xmin),ymax,z, lc} ;
Point(21) = {xmax+(xmax-xmin)-width_left,ymax,z, lc} ;
Point(22) ={xmax+(xmax-xmin),ymin+width_bottom,z,lc};
Point(23) = {xmax+(xmax-xmin)-width_left,ymin_lowPerm,z, lc};
Point(24) = {xmax+(xmax-xmin),ymin,z, lc};
Point(25)={xmax+(xmax-xmin)-width_left,ymin,z,lc};
...
Line(37) = {2, 25};
//+
Curve Loop(6) = {36, -24, 37, -33};
//+
Plane Surface(6) = {6};
//+
Curve Loop(7) = {31, 33, 34, 35};
//+
Plane Surface(7) = {7};
//+
Curve Loop(8) = {30, 31, 32, 29};
//+
Plane Surface(8) = {8};
...
Point(301) = {xmax+(xmax-xmin), ymin+width_bottom/2, z, lc};
Line(300)={300,301};
Field[11] = Attractor;
...
Physical Volume("rechargeZone") = {1, 8};
Physical Volume("dischargeZone") = {2,3,4,5, 6, 7};
Physical Surface("frontAndBack") = {1:8, 13, 17, 21, 25, 31, 35, 39, 43};
Physical Surface("wall") = {10, 16, 12, 18, 30, 29, 28, 26, 32, 41, 40, 38}; //12
Physical Surface("bottom") = {14, 20, 24, 33, 37};
Physical Surface("top") = {42, 27, 9};

Color Gray{Surface{1:8, 13, 17, 21, 25, 31, 35, 39, 43};}
Color Purple{Surface{10, 16, 12, 18, 30, 29, 28, 26, 32, 41, 40, 38};}
Color Red{Surface{14, 20, 24, 33, 37};}
Color Blue{Surface{42, 27, 9};}
